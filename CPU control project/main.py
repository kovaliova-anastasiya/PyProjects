...
class Application(tk.Tk, ConfigureWidgets):
    ...
    def set_ui(self):
        exit_butt = ttk.Button(self, text='Exit', command=self.app_exit)  # вызов,те скобки её здесь записывать не нужно
        ...
        self.combo_win.bind('<<ComboboxSelected>>', self.choise_combo)  # обработка событий
    ...
    def make_bar_cpu_usage(self):
        ...
        # создадим прогрессБар и метку для отображения загрузки оперативной памяти
    ...
    def enter_mouse(self, event):
        if self.combo_win.current() == 0 or 1:
            self.geometry('')  # означает, что геометрия окна по умоланию, остаётся такой, какая есть сейчас
    ...
    def choise_combo(self, event):
        if self.combo_win.current() == 2:
            self.enter_mouse('')  # чтобы окно открылось, как будто на него навели мышь
            #  отвязываем два события наведения мыши на класс Tk
            self.unbind_class('Tk', '<Enter>')
            self.unbind_class('Tk', '<Leave>')
            self.combo_win.unbind('<<ComboboxSelected>>')
            self.after_cancel(self.wheel)  # отвязываем автоматическое обновление окна
            # после этого шага окно застывает на каких-то показателях
            self.clear_win()
            self.update()
